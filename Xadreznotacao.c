...
#define CAMINHO_PARTIDA "partidanotacao.txt"
...
int moverstd(char org[3], char dest[3], Peca tabuP[8][8]){
    ...
    Peca obj_dest;
    obj_dest = tabuP[lin_dest][col_dest];
...
void promocao(Peca *p, char apromo){
...
void decodificar(int vez, char jog[10], char inp_org[3], char inp_dest[3], char *apromo, Peca tabuP[8][8]){
    char *pos_dest;
    Peca possiveis[11];
    char tipo_org;

    for (int i = strlen(jog) - 1; i >= 0; i--){
        if (isdigit(jog[i])) {
            pos_dest = &jog[i - 1];
            strncpy(inp_dest, pos_dest, 2);
            break;
        }
        if (isupper(jog[i])) *apromo = jog[i];
    }
    ...
    for (int i = 0; i < 8; i++){
        for (int j = 0; j < 8; j++){
            if (toupper(tabuP[i][j].tipo) == tipo_org && tabuP[i][j].cor == vez && existemoveval(&tabuP[i][j], stdtoi(*(pos_dest + 1)), stdtoi(*pos_dest))){
                possiveis[qtd] = tabuP[i][j];
                qtd++;
            }
        }
    }
    if (qtd > 1){
        for (int i = 0; i < qtd; i++){
            if (isalpha(difer)){
                if (stdtoi(difer) == possiveis[i].pos[1]){
                    sprintf(inp_org, "%c%c", itostd(possiveis[i].pos[1], 'c'), itostd(possiveis[i].pos[0], 'l'));
                }
            }else{
                if (stdtoi(difer) == possiveis[i].pos[0]){
                    sprintf(inp_org, "%c%c", itostd(possiveis[i].pos[1], 'c'), itostd(possiveis[i].pos[0], 'l'));
                }
            }
        }
    }else{
        sprintf(inp_org, "%c%c", itostd(possiveis[0].pos[1], 'c'), itostd(possiveis[0].pos[0], 'l'));
    }
}

void castle(char jog[], int vez, Peca tabuP[8][8]){
    Peca rei;
    for (int i = 0; i < 8; i++){
        for (int j = 0; j < 8; j++){
    ...
    if (strcmp(jog, "O-O") == 0){
        moverpos(rei.pos, (int []){rei.pos[0], 6}, tabuP);
        moverpos((int []){rei.pos[0], 7}, (int []){rei.pos[0], 5}, tabuP);
    }else{
        moverpos(rei.pos, (int []){rei.pos[0], 2}, tabuP);
        moverpos((int []){rei.pos[0], 0}, (int []){rei.pos[0], 3}, tabuP);
    }
}

int main (){
    FILE *mapa_t = fopen(CAMINHO_MAPA, "r");
    FILE *partida = fopen(CAMINHO_PARTIDA, "r");
    char tabuC[9][19];
    Peca tabuP[8][8];
    Peca tempP;
    ...
    Peca rei;
    int vez = BRANCO;
    char num_jog[10], inp_org[3], inp_dest[3], jog[10];
    ...
    while (1){
        ...
        //ler input
        do{
            fscanf(partida, "%s", jog);
        }while (jog[strlen(jog) - 1] == '.');

        printf("%s\n", jog);

        if (strcmp(jog, "O-O") == 0 || strcmp(jog, "O-O-O") == 0){
            castle(jog, vez, tabuP);
            if (vez == BRANCO) vez = PRETO;
            else vez = BRANCO;
            atlztabuC(tabuC, tabuP);
            continue;
        }
        decodificar(vez, jog, inp_org, inp_dest, &apromo, tabuP);
